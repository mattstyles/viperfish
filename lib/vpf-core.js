...
vpf.core = {
    ...
    authGithub: function() {
        ...
        if ( auth ) {
            console.log( 'Got github credentials from process.env --- attempting to authenticate with github' );
            vpf.core.github.authenticate( auth );
        } else {
            console.warn( 'No github credentials in environment vars --- Unauthorised access to github' );
        }
    },
    ...
    setViewPath: function() {
        require('fs').readdir( process.env.PWD + '/public/templates', function( err, files) {
            if ( err ) {
                return;
            }
            ...
            if (files !== undefined) {
                ...
                vpf.set('views', process.env.PWD + '/public/templates');
            }
        });
    }

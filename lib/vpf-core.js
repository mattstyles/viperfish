...
vpf.core = {
    ...
    /**
     * setViewPath
     * Finds a template directory if one exists in the root and uses it
     * // @todo Tidy this up and sort it out - messy and no error handling at the moment
     */
    setViewPath: function() {
        fs.readdir( process.cwd() + '/public/templates', function( err, files) {
            if ( err ) {
                console.log( 'Attempting to find custom templates path --- something went wrong!' );
                return;
            }

            // If files exist in the read directory then set the path
            // No current error handling for falling back to default templates when custom ones dont exist
            if (files !== undefined) {
                console.log( 'Custom templates found - setting view path' );
                vpf.set('views', process.cwd() + '/public/templates');
            }
        });
    }
